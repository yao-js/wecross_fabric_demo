...
public class CommitTxThread extends Thread{
    ...
    public CommitTxThread(String name, String accountName, String password, String chainType, Integer defaultAccountID, String txId, String[] chainResources) {
        this.name = name;
...
//                        设置当前默认账号为fabric01中的Org1.msp
            ...
            Response response = weCrossRPC.commitXATransaction(this.txId, chainResources).send();
            if (response.getErrorCode() != 0) {
                setFinished(false);
                setErrorMessage(response.getMessage());
                System.out.println("跨链事务提交失败！" + response.getMessage());
            } else {
                setFinished(true);
                System.out.println("跨链事务提交成功！");
            }
