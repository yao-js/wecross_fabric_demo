...
public class LockResourceThread extends Thread{
    ...
    public LockResourceThread(String name, String accountName, String password, String chainType, Integer defaultAccountID, String txId, String[] chainResources) {
        this.name = name;
...
//                        设置当前默认账号为fabric01中的Org1.msp
            ...
            Response response = weCrossRPC.startXATransaction(this.txId, this.chainResources).send();
            if (response.getErrorCode() != 0) {
                setFinished(false);
                setErrorMessage(response.getMessage());
                System.out.println("跨链锁定资源失败！" + response.getMessage());
            } else {
                setFinished(true);
                System.out.println("跨链锁定资源成功！");
            }
